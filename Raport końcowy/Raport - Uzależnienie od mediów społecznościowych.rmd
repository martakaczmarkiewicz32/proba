...
demografia <- dane_clean %>%
  select(Plec, Poziom_akademicki, Kraj) %>%
  gather(key = "Zmienna", value = "Kategoria") %>%
  filter(Zmienna == "Kraj") %>%          # ← TO JEST KLUCZ
  count(Zmienna, Kategoria) %>%
  mutate(Procent = round(n / sum(n) * 100, 1)) %>%
  arrange(desc(n))
...
**Wnioski**
Wartość p jest znacznie pozniżej progu 0,05, co pozwala na odrzucenie hipotezy zerowej na rzecz hipotezy alternatywnej. Istnieje statystyczna zależność między czasem spędzonym w mediach społecznościowych a oceną zdrowia psychicznego.
Współczynnik korelacji rang Spearmana wynosi -0,802, co wskazuje na silną ujemną korelację. Oznacza to, że wraz ze wzrostem liczby godzin spędzanych w mediach społecznościowych, ocena zdrowia psychicznego wyraźnie spada.
...
**Wnioski**
Wartość p jest wyższa niż próg 0,05, oznacza to, że mamy brak podstaw do odrzucenia hipotezy zerowej. Różnica w poziomie uzależnienia między kobietami a mężczyznami nie jest istotna statystycznie. Wynik ten pokazuje, że płeć nie jest czynnikiem różniującym skłonność do uzależnienia od mediów społecznościowych. Jak również w pierwszej tabeli pokazano, że mediana jest taka sama.
...
**Wnioski**
Wartość p jest niższa od progu 0,05, co pozwala na odrzucenie hipotezy zerowej na rzecz hipotezy alternatywnej, istnieje zależność między płcią a wyborem platfomy.
...
**Wnioski**
Wartość p jest niższa od progu 0,05, co pozwala na odrzucenie hipotezy zerowej na rzecz hipotezy alternatywnej. Istnieje zależność między płcią a wyborem dominującej platformy społecznościowej. Można zauważyć, że kobiety wybierają częściej platformy skupiające się na treściach wizualnych, a mężczyźni preferują platformy o charakterze informacyjnym. Test wykazuje, że w większości studenci na różnych etapach edukacji wykazują różnice w poziomie uzależnienia. 
...
**Wnioski**
Wartość p jest znacznie poniżej progu 0,05, co pozwala na odrzucenie hipotezy zerowej na rzecz hipotezy alternatywnej. Związek między czasem spędzonym w mediach społecznościowych a długością snu jest statystycznie istotny. Współczynnik korelacji Spearmana wynosi -0,814 co wskazuje na silną korelację ujemną, czyli im więcej czasu studenci spędzają w mediach społęcznościowych, tym mniej godzin przeznaczają na sen.
...
**Wnioski**
Wartość p jest znacznie poniżej progu 0,05, co pozwala na odrzucenie hipotezy zerowej na rzecz hipotezy alternatywnej, istnieje statystycznie istotna różnica poziomu uzależnienia między osobami, które deklarują negatywny wpływ mediów społecznościowych w nauce, a tymi, które go nie zauważają. Można także zauważyć, że najbardziej narażoną grupą są najmłodsi studenci jak i fakt, że grupa, która uważa, że miedia społecznościowe wpływają na ich wyniki w nauce jest prawie dwa razy większa niż grupa, w której uważają odwrotnie.
...
studentów. Zostały przebadane różne czynniki, które wpływają na dobrostan studentów, m.in.: czas spędzony w mediach społecznościowych czy konflikty.


Zgodnie z celem projektu zostały przeprowadzone analizy, z których wynikają następujące wnioski:

- Badana grupa jest zróżnicowana pod względem pochodzenia, najlczniejsza grupa pochodzi z Indii i USA

- Średni czas spędzany w mediach społecznościowych wynosi około 5 godzin, co zajmuje dużą część doby.


- Blisko 30% studentów należy do grupy wysokiego ryzyka.
...
Spearmana między czasem użytkowania a zdrowiem psychicznym. Im więcej czasu studenci spędzają w mediach tym niżej oceniają swoje zdrowie psychiczne.
...
Podsumuwując wyniki analizy można stwierdzić, że media społęcznościowe są dużym wyzwaniem dla życia młodych dorosłych. Silnie wpływają na ich sen, zdrowie psychiczne jak i wyniki w nauce co też świadczy o wyzwaniu dla obecnej edukacji.
